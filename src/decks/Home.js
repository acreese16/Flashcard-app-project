...
function Home () {
    ...
    const deckList = decks.map((deck) => (
        <li key={deck.id} className="list-group-item flex-column align-items-start">
            ...
            <Link to={`/decks/${deck.id}`} className="btn btn-secondary mr-2" title="Edit Deck" >
                View
            </Link>
            <Link to={`/decks/${deck.id}/study`} className="btn btn-primary mr-2" title="Study Deck" >
                Study
            </Link>
            <button className="btn btn-danger justify-content-end" onClick={() => deleteHandler(deck.id)} title="Delete Deck" >
                Delete
            </button>
        </li>
    ))
